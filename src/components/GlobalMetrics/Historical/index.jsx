...
import { fetchHistoricalGlobalMetrics } from '../../../actions/GlobalMetrics/historicalGlobalMetricsActions'
import * as d3 from 'd3'
import moment from 'moment'

const margin = 20

class HistoricalGlobalMetrics extends Component {

  componentDidMount() {
    this.props.dispatch(fetchHistoricalGlobalMetrics())
  }
  ...
  render() {
    ...
    if (loading) {
      ...
      const data = this.props.data.quotes
      ...
      const x = d3.scaleLinear()
        .domain(d3.extent(data, d => moment(d.timestamp))) //domain: [min,max] of a
        .range([margin, w])

      //y scale
      const y = d3.scaleLinear()
        .domain([0, d3.max(data, d => d.btc_dominance)]) // domain [0,max] of b (start from 0)
        .range([h, margin])

      //line generator: each point is [x(d.a), y(d.b)] where d is a row in data
      // and x, y are scales (e.g. x(10) returns pixel value of 10 scaled by x)
      const line = d3.line()
        .x(d => x(moment(d.timestamp)))
        .y(d => y(d.btc_dominance))
      ...
      const yTicks = y.ticks(5).map(d => (
        y(d) > 10 && y(d) < h ?
          <g transform={`translate(${margin},${y(d)})`}>
            ...
            <line className='gridline' x1='0' x1={w - margin} y1='0' y2='0'
...
const mapStateToProps = state => {
  return {
    ...state.globalMetrics.historical
  }
}
